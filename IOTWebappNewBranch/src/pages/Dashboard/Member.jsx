...
import { getMemberOfStation, addMemberToStation } from "../../services/memberService";
...
const Member = (props) => {
  const { image, full_name, role } = props.props;

  return (
    <div className="flex flex-col items-center hover:scale-105 cursor-pointer">
      <img className="rounded-full size-10" src={image || defaultImage} alt="" />
...
const MemberAddForm = ({ onClose }) => {
  ...
  const handleForm = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await addMemberToStation(selectedStation, formData);
      if (res.status === 200) {
        console.log("create ok");
        window.location.reload();
      }
    } catch (error) {
      setErrMessage("Can not add this user.");
      console.log(error);
    }
    setIsLoading(false);
  }
    ...
    <Loading isLoading={isLoading}>
      <div className="flex flex-col gap-y-3 p-8 bg-white w-[40rem] rounded-md overflow-auto h-[95%]">
        <div className="flex justify-between items-center">
          <p className="text-2xl font-bold">Add member</p>
...
const MemberList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getMemberOfStation(selectedStation);
        if (res.status === 200) {
          setMembers(res.data);
        }
      } catch (err) {
  ...
  return (
    <div className="flex flex-col p-4 gap-y-2 bg-gray-100 rounded-lg">
      ...
      <Modal isOpen={showMemberForm} onClose={closeMemberForm}>
        <MemberAddForm onClose={closeMemberForm} />
      </Modal>
    </div>
  );
};
